...
server <- function(input, output, session) {
  ...
  # =================== STATISTIK INFERENSIA DOWNLOAD HANDLERS ===================
  
  output$download_inference_plot <- downloadHandler(
    filename = function() {
      paste0("statistik_inferensia_plot_", Sys.Date(), ".jpg")
    },
    content = function(file) {
      p <- NULL
      if (input$inference_type == "one_t") {
        p <- ggplot(values$data, aes_string(x = input$inference_var)) +
          geom_histogram(bins = 30, fill = "#9EF09E", alpha = 0.7) +
          geom_vline(xintercept = input$test_value, color = "#FF5005", linetype = "dashed", size = 1) +
          geom_vline(xintercept = mean(values$data[[input$inference_var]], na.rm = TRUE), color = "#51CD37", size = 1) +
          theme_minimal() +
          labs(title = "Uji t Satu Sampel")
      } else if (input$inference_type == "two_t") {
        data_combined <- data.frame(
          values = c(values$data[[input$inference_var]], values$data[[input$inference_var2]]),
          group = c(rep("Variabel 1", length(values$data[[input$inference_var]])), 
                    rep("Variabel 2", length(values$data[[input$inference_var2]])))
        )
        p <- ggplot(data_combined, aes(x = group, y = values)) +
          geom_boxplot(fill = "#9EF09E", alpha = 0.7) +
          theme_minimal() +
          labs(title = "Uji t Dua Sampel")
      } else if (input$inference_type %in% c("one_prop", "two_prop")) {
        # Mirip dengan plot di renderPlotly
        if (input$inference_type == "one_prop") {
          var_data <- values$data[[input$inference_var]]
          success_count <- sum(var_data > input$cutpoint, na.rm = TRUE)
          fail_count <- sum(var_data <= input$cutpoint, na.rm = TRUE)
  ...
  output$download_inference_results <- downloadHandler(
    filename = function() {
      paste0("statistik_inferensia_hasil_", Sys.Date(), ".pdf")
    },
    content = function(file) {
      temp_md <- tempfile(fileext = ".md")
      test_result <- get_inference_results()
      hasil_uji <- capture.output(if (input$inference_type %in% c("anova1", "anova2")) summary(test_result) else test_result)
      interpretasi <- output$inference_interpretation()
      lines <- c(
        "# Hasil Uji Statistik Inferensia",
        "",
        "### Hasil Uji Statistik:",
        hasil_uji,
        "",
      ...
      tryCatch({
        ...
        file.create(file)
        writeLines("Gagal membuat PDF. Pastikan LaTeX (TinyTeX) sudah terinstal di sistem Anda.", file)
      })
    }
  )
  
  output$download_inference_page <- downloadHandler(
    filename = function() {
      paste0("halaman_statistik_inferensia_", Sys.Date(), ".pdf")
    },
    content = function(file) {
      temp_md <- tempfile(fileext = ".md")
      test_result <- get_inference_results()
  ...
  # =================== REGRESI LINEAR BERGANDA DOWNLOAD HANDLERS ===================
  
  output$download_regression_plot <- downloadHandler(
    filename = function() {
      paste0("regresi_linear_plot_", Sys.Date(), ".jpg")
    },
    content = function(file) {
      req(input$reg_dependent, input$reg_independent)
      formula_str <- paste(input$reg_dependent, "~", paste(input$reg_independent, collapse = " + "))
      ...
      p <- ggplot(plot_data, aes(x = fitted, y = residuals)) +
        ...
        geom_smooth(method = "loess", color = "#51CD37", se = FALSE) +
        theme_minimal() +
        labs(title = "Residu vs Nilai Fitted", x = "Nilai Fitted", y = "Residu")
      ggsave(file, plot = p, device = "jpeg", width = 8, height = 5)
    }
  )
  ...
  output$download_regression_results <- downloadHandler(
    filename = function() {
      paste0("regresi_linear_hasil_", Sys.Date(), ".pdf")
    },
    content = function(file) {
      req(input$reg_dependent, input$reg_independent)
      formula_str <- paste(input$reg_dependent, "~", paste(input$reg_independent, collapse = " + "))
      model <- lm(as.formula(formula_str), data = values$data)
      temp_md <- tempfile(fileext = ".md")
      hasil_regresi <- capture.output(summary(model))
      asumsi <- get_regression_assumptions(model)
      hasil_asumsi <- c()
      if (!is.null(asumsi$linearity)) hasil_asumsi <- c(hasil_asumsi, "1. UJI LINEARITAS", asumsi$linearity, "")
      if (!is.null(asumsi$independence)) hasil_asumsi <- c(hasil_asumsi, "2. UJI INDEPENDENSI (Durbin-Watson)", asumsi$independence, "")
      if (!is.null(asumsi$normality)) hasil_asumsi <- c(hasil_asumsi, "3. NORMALITAS RESIDU (Shapiro-Wilk)", asumsi$normality, "")
      if (!is.null(asumsi$homoscedasticity)) hasil_asumsi <- c(hasil_asumsi, "4. UJI HOMOSCEDASTISITAS (Breusch-Pagan)", asumsi$homoscedasticity, "")
      if (!is.null(asumsi$multicollinearity)) hasil_asumsi <- c(hasil_asumsi, "5. UJI MULTIKOLINEARITAS (VIF)", asumsi$multicollinearity, "")
      interpretasi <- output$regression_interpretation()
      lines <- c(
  ...
  output$download_regression_page <- downloadHandler(
    filename = function() {
      paste0("halaman_regresi_linear_", Sys.Date(), ".pdf")
    },
    content = function(file) {
      req(input$reg_dependent, input$reg_independent)
      formula_str <- paste(input$reg_dependent, "~", paste(input$reg_independent, collapse = " + "))
      ...
      interpretasi <- output$regression_interpretation()
      lines <- c(
        "# Halaman Regresi Linear Berganda",
        "",
        "### Ringkasan Model:",
        hasil_regresi,
        "",
